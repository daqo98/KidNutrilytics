...
def ploting_distribution(base_pivot):
    ...
    """
    x = porcentajes_mes['FechaValoracionNutricional_MesAño'].unique().strftime('%Y-%m')
    y1 = porcentajes_mes[porcentajes_mes['EstadoPesoTalla_New'] == 'Desnutricion']['Porcentajes']
    fig = px.line(porcentajes_mes, x=x, y=y1, color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
    names = cycle(['Malnutrition 2017', 'Malnutrition 2018', 'Malnutrition 2019'])
    #fig.for_each_trace(lambda x : x.update(name = next(names)))
    fig.update_layout(xaxis_title="Month", yaxis_title="Percentage")
    fig.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)', 'paper_bgcolor': 'rgba(0, 0, 0, 0)'})
